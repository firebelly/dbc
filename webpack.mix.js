...
mix
  ...
  // Styles
  .sass(src`styles/main.scss`, 'styles')
  // JavaScript
  ...
  .then(function () {
    ...
    config.defaults.css = manifest['/styles/main.css']
    config.defaults.js = manifest['/scripts/main.js']
    fs.writeFileSync('./_config.yml', yaml.safeDump(config))
    childProcess.exec('bundle exec jekyll serve --watch')
  })
...
mix.webpackConfig({
 plugins: [
   new SVGSpritemapPlugin('_assets/svgs/*.svg', {
     output: {
